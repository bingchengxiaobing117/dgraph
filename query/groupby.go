...
func (sg *SubGraph) formResult(UIDPack *pb.UidPack) (*groupResults, error) {
	...
	for _, child := range sg.Children {
		...
		if child.DestUIDs.IsEmpty() {
			...
			x.Check(codec.Intersect(child.SrcUIDs, codec.UIDSetFromPack(UIDPack)).Iterate(func(srcUID uint64) error {
			...
			// TODO: Optimize with Intersection iterator
			x.Check(codec.Intersect(child.SrcUIDs, codec.UIDSetFromPack(UIDPack)).Iterate(func(srcUID uint64) error {
				return codec.UIDSetFromPack(child.uidMatrix[srcUID]).Iterate(func(adjUID uint64) error {
